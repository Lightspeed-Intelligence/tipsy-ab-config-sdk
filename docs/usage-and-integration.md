...
> **主键 ID 分配标准（接入方需了解）**：平台所有 `config_key.id` / `config_version.id` /
> `release_full.id` 等 BIGSERIAL 主键被切成三段——`[1, 100万)` 为早期遗留自增段、
> `[100万, 200亿)` 为「SQL 手工指定 id」保留段（seed / prompt 迁移 / 配置 / 实验批量注入）、
> `[200亿, ∞)` 为运行时自增段。SDK 侧把 `key_id` / `version_id` 当 int64 wire 值读取（见
> §3.5、§9），不分配主键；但凡需要**通过 SQL 批量注入**配置 / 实验 / 灰度数据（指定 `id`），
> **必须**在 tipsy-ab-config 平台仓库的统一台账 `docs/id-allocation-ledger.md` 里查可用段、
> 分配后回填，避免与运行时自增段或既有手工段冲突。完整规则见 ab-config 仓库 README
> `## Production deployment: DB requirements` → `### Primary-key ID allocation convention`。
