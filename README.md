...
- **Java SDK**（Maven 多模块，Java 21）— 见 [`sdk/java/README.md`](sdk/java/README.md)
  ...
  - `sdk/java/tipsy-auth` — JWT 签名小工具（`io.tipsy:tipsy-auth`）
  - `sdk/java/tipsy-abconfig` — 主 SDK（`io.tipsy:tipsy-abconfig`），含可选的 `io.tipsy.abconfig.web` web 集成子包
...
```bash
cd sdk/java
mvn -q -DskipTests install   # 安装到本地 .m2（io.tipsy:tipsy-abconfig:0.1.0）
```
...
完整可运行示例参见 [`sdk/java/example/src/main/java/io/tipsy/abconfig/example/Main.java`](sdk/java/example/src/main/java/io/tipsy/abconfig/example/Main.java)，
